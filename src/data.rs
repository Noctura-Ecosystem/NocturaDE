...
impl CompositorHandler for State {
    fn compositor_state(&mut self) -> &mut CompositorState { // for new apps wanting a surface (window)
        &mut self.compositor_state
    }
    fn commit(&mut self, surface: &WlSurface) { // when an app requests to draw onto its surface
        on_commit_buffer_handler::<Self>(surface);
    }
    fn client_compositor_state<'a>(&self, client: &'a Client) -> &'a CompositorClientState { // for smithay managing identification
        let data = client.get_data::<ClientData>().unwrap();
...
impl SeatHandler for State {
    ...
    fn seat_state(&mut self) -> &mut SeatState<Self> { // if smithay wants to check the seat_state
        &mut self.seat_state
    }
...
impl XdgShellHandler for State {
    fn xdg_shell_state(&mut self) -> &mut XdgShellState { // when smithay wants xdg_shell_state
        &mut self.xdg_shell_state
    }

    fn new_toplevel(&mut self, surface: ToplevelSurface) { // when new apps want to open
